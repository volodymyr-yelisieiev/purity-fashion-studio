...
import { verifyLiqPayCallback, parseLiqPayCallback, mapLiqPayStatus } from '@/lib/liqpay'
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!verifyLiqPayCallback(data, signature)) {
      console.error('LiqPay signature verification failed')
      return NextResponse.json(
    ...
    if (orderStatus === 'paid') {
      ...
      console.log(`✓ Order ${order.orderNumber} marked as paid (LiqPay)`)
    }
