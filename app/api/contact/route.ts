...
export async function POST(request: Request) {
  try {
    ...
    const adminEmail = siteConfig.url !== 'http://localhost:3000' 
    ...
    if (features.email) {
      ...
      const userHtmlContent = isBooking
        ? `
          ...
          ${preferredDate ? `<p><strong>Requested Date:</strong> ${preferredDate}</p>` : ''}
          ${preferredTime ? `<p><strong>Requested Time:</strong> ${preferredTime}</p>` : ''}
      ...
      await sendEmail({
        ...
        text: `Thank you for your ${isBooking ? 'booking request' : 'message'}! We will get back to you soon.`,
      })
    }

    // Log the contact/booking (in production, you might store this in a database)
    console.log(`${type === 'booking' ? 'Booking' : 'Contact'} request received:`, {
      type,
      name: `${firstName} ${lastName}`,
      email,
      phone,
